...
$major_id = intval($_GET['id']); // ID của ngành đang sửa
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($ten_nganh)) {
        ...
        $check = fetchOne($conn, "SELECT id FROM majors WHERE (ma_nganh = ? OR ten_nganh = ?) AND id != ?", "ssi", [$ma_nganh, $ten_nganh, $major_id]);
        if ($check) {
            $_SESSION['error'] = "Mã ngành hoặc Tên ngành đã tồn tại!";
        } else {
            $sql = "UPDATE majors SET ma_nganh = ?, ten_nganh = ? WHERE id = ?";
            if (executeQuery($conn, $sql, "ssi", [$ma_nganh, $ten_nganh, $major_id])) {
...
// Lấy thông tin ngành học hiện tại
$major = fetchOne($conn, "SELECT * FROM majors WHERE id = ?", "i", [$major_id]);
if (!$major) {
    header("Location: majors.php");
    exit();
}
    ...
    <div class="col-lg-8 mx-auto">
        ...
        <div class="card shadow">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="fas fa-edit me-2"></i>Sửa Ngành Học</h5>
            </div>
            <div class="card-body">
                <form method="POST">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="ma_nganh" name="ma_nganh" 
                               placeholder="VD: CNTT"
                               value="<?php echo htmlspecialchars($major['ma_nganh']); ?>">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="ten_nganh" name="ten_nganh" 
                               ...
                               value="<?php echo htmlspecialchars($major['ten_nganh']); ?>">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentMajorId = <?php echo $major_id; ?>; // Lấy ID từ PHP
    ...
    maNganhInput.addEventListener('input', function() {
        debouncedCheckMaNganh('ma_nganh', this.value.trim(), maNganhError, submitButton, currentMajorId);
    });
    
    tenNganhInput.addEventListener('input', function() {
        debouncedCheckTenNganh('ten_nganh', this.value.trim(), tenNganhError, submitButton, currentMajorId);
    });
});
</script>
